...
fun isAlphaNumericWithLengthLessThan30(text: String) = Pattern.compile("^[a-zA-Z0-9 ]{0,30}\$",Pattern.CASE_INSENSITIVE).matcher(text).find()
...
fun isValidName(text: String)  = Pattern.compile("^[a-zA-Z0-9 _ºñ]{2,10}$", Pattern.CASE_INSENSITIVE).matcher(text).find()
fun isValidDescription(text: String)  = Pattern.compile("^[a-zA-Z0-9 /_º+*?¿,.ñ]{0,30}$", Pattern.CASE_INSENSITIVE).matcher(text).find()
